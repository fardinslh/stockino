      ...
      <aside className="stockino-drawer stockino-supplier-drawer" role="dialog" aria-modal="true" aria-labelledby="stockino-supplier-detail-title">
        ...
        {supplier && <div className="stockino-supplier-detail-body">
          <section className="stockino-supplier-profile">
            ...
            <dl><div><dt>تلفن</dt><dd dir="ltr">{supplier.phone || '—'}</dd></div><div><dt>ایمیل</dt><dd dir="ltr">{supplier.email || '—'}</dd></div><div><dt>زمان تأمین</dt><dd>{supplier.lead_time_days === null ? 'نامشخص' : `${supplier.lead_time_days.toLocaleString('fa-IR')} روز`}</dd></div><div><dt>وب‌سایت</dt><dd dir="ltr">{supplier.website ? <a href={supplier.website} target="_blank" rel="noreferrer">{supplier.website}</a> : '—'}</dd></div></dl>
